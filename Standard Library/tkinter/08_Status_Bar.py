...
status = Label(
    root, text=f'Image 1 of {len(image_list)}', bd=1, relief=SUNKEN, anchor=E)
...
def next(img_num):
    ...
    status = Label(
        root, text=f'Image {img_num} of {len(image_list)}', bd=1, relief=SUNKEN, anchor=E)
...
btn_fwd.grid(row=1, column=2, pady=10)
status.grid(row=2, column=0, columnspan=3, sticky=W+E)
root.mainloop()
